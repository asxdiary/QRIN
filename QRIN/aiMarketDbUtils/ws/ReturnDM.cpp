...
Matrix* ReturnDM_gen(CompanyUniverse* univ, Db* dmdb, UtilDate* d, int K) {
...
Db* ReturnDM_gen_dmdb(CompanyUniverse* univ, UtilDate* da, UtilDate* db) {
   list<UtilDate*> dL;
   Db* dmdb = NULL;
...
int ReturnDM_main() {
   ...
   dmdb = ReturnDM_gen_dmdb(univ, da, db);

   dm = new unordered_map<UtilDate*, Matrix*>();
   for(d=da;d->isBefore(db);d=d->next_eom()) {
      X = ReturnDM_gen(univ, dmdb, d,K);
      if(X==NULL) continue;
      (*dm)[d] = X;
   }

   ForecastUtils_fprint_split(dm, util_join(".","./data/returndm",K));
